...
class generate_shipping_quotes(models.TransientModel):
    ...
    name = fields.Char(string='Batch No',size=64, defaults= lambda self, cr, uid, context: self.pool['ir.sequence'].next_by_code(cr, uid, 'print.packing.slip', context=context))
            ...
            for picking_id in picking_ids:
                ...
                if picking_data.state != 'done':
                    if picking_data.shipping_type.lower() == 'usps':
                        carrier_ids = self.env['delivery.carrier'].search([('service_output','=',picking_data.service_type_usps),('is_usps','=',True),('container_usps','=',picking_data.container_usps)])
                    elif picking_data.shipping_type.lower() == 'fedex':
                        carrier_ids = self.env['delivery.carrier'].search([('service_output','=',picking_data.service_type_fedex),('is_fedex','=',True)])
                    elif picking_data.shipping_type.lower() == 'ups':
                        carrier_ids = self.env['delivery.carrier'].search([('service_output','=',picking_data.service_type_ups),('is_ups','=',True)])
                    if not carrier_ids:
                        picking_data.write({'is_faulty':True,'error_for_faulty':'Shipping service output settings not defined'})
                        continue
