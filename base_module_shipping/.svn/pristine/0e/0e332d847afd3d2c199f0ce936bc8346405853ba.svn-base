...
#        try:
        ...
        if shippingresp_lnk.type.lower() == 'usps' and not ('usps_active' in context.keys()):
            ...
            carrier_ids = self.pool.get('delivery.carrier').search(cr,uid,[('service_output','=',picking_data.service_type_usps),('is_usps','=',True),('container_usps','=',picking_data.container_usps),('size_usps','=',picking_data.size_usps)])
        elif shippingresp_lnk.type.lower() == 'fedex':
            result = self.generate_fedex_tracking_no(cr,uid,shippingresp_lnk.picking_id,context)
            carrier_ids = self.pool.get('delivery.carrier').search(cr,uid,[('service_output','=',picking_data.service_type_fedex),('is_fedex','=',True)])
        elif shippingresp_lnk.type.lower() == 'ups':
            result = self.generate_ups_tracking_no(cr,uid,shippingresp_lnk.picking_id,context)
            carrier_ids = self.pool.get('delivery.carrier').search(cr,uid,[('service_output','=',picking_data.service_type_ups),('is_ups','=',True)])
        if context.get('track_success',False):
            ...
            saleorder_obj.write(cr,uid,picking_data.sale_id.id,{'client_order_ref':context['tracking_no'], 'carrier_id':carrier_ids[0]})
...
class stock_picking(models.Model):
    ...
    def do_print_picking(self, cr, uid, ids, context=None):
        ...
        return self.pool.get("report").get_action(cr, uid, ids, 'shipping_teckzilla.shipping_report_picking', context=context)
...
#    product_qty = fields.Char(string='Qty', related='move_lines.product_qty')
    ...
    weight_package = fields.Float(string='Package Weight', digits_compute= dp.get_precision('Stock Weight'), help="Package weight which comes from weighinig machine in pounds")
    ...
    def generate_fedex_shipping(self, id, dropoff_type_fedex, service_type_fedex, packaging_type_fedex, package_detail_fedex, payment_type_fedex, physical_packaging_fedex, weight, shipper_postal_code,shipper_country_code,customer_postal_code,customer_country_code, sys_default=False,cust_default=False, error=True, context=None):
        '''
        This function is used to Get shipping rates for Fedex 
        parameters: 
            All the Fedex Shipping and packaging type parameters
        '''
        fedex_rate = self.return_fedex_shipping_rate(dropoff_type_fedex,service_type_fedex,packaging_type_fedex,package_detail_fedex,payment_type_fedex,physical_packaging_fedex,weight,shipper_postal_code,shipper_country_code,customer_postal_code,customer_country_code,sys_default,cust_default,error,context)
        sr_no = 9
    ...
    def create_attachment(self, vals):
        ...
        attach_id = attachment_pool.search([('res_id','=',ids[0]),('res_name','=',self.browse(self[0].id).name)])
        if not attach_id:
...
#            sys_default = 'USPS/First Class/Parcel/REGULAR/1'
        ...
        if product_shipping_ids:
            self._cr.execute('SELECT * FROM product_product_shipping WHERE weight <= %s and product_id=%s order by sequence desc limit 1' % (weight,product_id))
        else:
            categ_id = product_obj.browse(product_id).product_tmpl_id.categ_id.id
            self._cr.execute('SELECT * FROM product_category_shipping WHERE weight <= %s and product_categ_id=%s order by sequence desc limit 1'% (weight,categ_id))
        ...
        if res:
            if res[0]['shipping_type'] == 'USPS':
                ...
                sys_default = res[0]['shipping_type'] + '/' + service_type_usps + '/' + container_usps + '/' + size_usps + '/' + postage
    ...
    def get_min_shipping_rate(self,sale_id,product_shipping_ids,weight):
        ...
        shipper = Address(shop_address.name or shop_address.name, shop_address.street, shop_address.city, shop_address.state_id.code or '', shop_address.zip, shop_address.country_id.code, shop_address.street2 or '', shop_address.phone or '', shop_address.email, shop_address.name)
        ### Recipient
        ...
        for product_shipping_id in product_shipping_ids:
            ...
            if product_shipping_data:
                if product_shipping_data.shipping_type== 'USPS':
                    ...
                    try:
                        all_shipping_res = self.get_endicia_rates(False,weight,shipper,receipient,context.get('cust_default',False),usps_rate_sys_default,context.get('error',False),context)
                        usps_rate = []
                        shipping_usps_rate = {}
                        ...
                        try:
                            if cust_address.country_id.code == 'US':
                                ...
                                measurement = 'KGS'
                            ups = shippingservice.UPSRateRequest(ups_info, pickup_type_ups, service_type_ups, packaging_type_ups, weight, shipper, receipient, False, ups_rate_sys_default, measurement,True)
                            ups_response = ups.send()
                    ...
                    error_required = True
#                    try:
                    fedex_rate = self.return_fedex_shipping_rate(dropoff_type_fedex,service_type_fedex,packaging_type_fedex,package_detail_fedex,payment_type_fedex,physical_packaging_fedex,weight,shipper.zip,shipper.country_code,receipient.zip,receipient.country_code,fedex_rate_sys_default,False,error_required,context)
